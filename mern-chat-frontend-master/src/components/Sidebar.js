...
function Sidebar() {
    ...
    return (
        <>
            ...
            <h2>Members</h2>
            {members.map((member) => (
                <ListGroup.Item key={uuidv4()} style={{ cursor: "pointer" }} active={privateMemberMsg?._id == member?._id} onClick={() => handlePrivateMemberMsg(member)} disabled={member._id === user._id}>
                    <Row>
            ...
            {/* <Button variant="primary" > INFERENCE </Button>
            <Button variant="danger" onClick={handleClick}> WORDCLOUD </Button> */}
        </>
    );
}
